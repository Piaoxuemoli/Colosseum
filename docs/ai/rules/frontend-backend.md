...
Colosseum 是 Next.js App Router 项目。源码位于 `src/`，按职责分层：

- `src/frontend/` — 配置、观战、展示、前端状态
- `src/app/api/` — Route Handler，A2A 端点、GM 端点、配置查询端点
- `src/backend/` — 业务编排：orchestrator、agent、a2a-core、auth、match
- `src/platform/` — 基础设施：db、redis、llm、telemetry、core、engine、memory
...
- **import 约束**：`src/frontend/` 只 import `@/frontend/*` 和 `@/platform/*` 的纯类型/工具；禁止直接 import `@/backend/*`、 `@/platform/db`、 `@/platform/redis` 等持久化/服务端模块。

## 后端边界

- Route Handler 是 HTTP 边界，负责鉴权、schema 校验、错误响应和调用业务层。
- 业务协调放在 `src/backend/orchestrator/` 或相近层，不把复杂流程塞进 route 文件。
- DB 访问集中在 `src/platform/db/`，不在 UI 组件中 import。
...
- 新增游戏时，优先创建 `src/games/<game>/` 下完整模块。
