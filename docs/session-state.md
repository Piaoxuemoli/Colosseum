...
- Active spec: `docs/legacy/2026-05-06-colosseum-rewrite-design.md`
- Brief spec: `docs/legacy/2026-05-06-colosseum-rewrite-brief.md`
...
| 2026-06-13 | Deployment skill + archive stale docs | Passed | Created `.kimi-code/skills/deployment/SKILL.md`; moved the phase-4 deployment plan, phase-6 server-verify plan, phase-0-complete note, and 4 demo checklists into the then-archive `old/docs-archive/` (since deleted); added its README; updated `AGENTS.md` with deployment section. `npm run lint` + `npm run typecheck` + `npm run build` passed. |
...
| 2026-06-14 | AI IDE deployment workflow sync gate | Passed | Synced Kimi deployment Skill routing into Cursor/Claude commands and rule routers; updated AGENTS and docs/rules README. Verified Cursor/Claude deploy command bodies match and deployment routes are discoverable with rg; `npm run lint` passed. |
...
- 2026-06-13 项目结构重组完成：源码迁入 `src/{app,frontend,backend,platform,games}`，归档整合到 `archive/old/`，`tsconfig.json` paths 改为 `@/*` → `./src/*`，`ops/deploy/Dockerfile` 同步复制 `src/platform/db/` 与 `src/platform/env.ts`；纲领文档 `AGENTS.md`、`docs/rules/project-context.md`、`docs/rules/frontend-backend.md`、`.kimi-code/skills/deployment/SKILL.md`、`README.md`、`.cursor/.claude rules` 已更新。
...
- 2026-06-14 AI IDE 部署流程同步完成：新增 `.cursor/.claude` 的 `deploy-production` 命令与 `deployment-router`，均回读 `.kimi-code/skills/deployment/SKILL.md` 作为权威部署流程；`AGENTS.md` 与 `docs/rules/README.md` 已补充部署/运维路由信息。
