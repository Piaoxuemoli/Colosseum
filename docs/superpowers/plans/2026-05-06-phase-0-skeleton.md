...
- **src/platform/env.ts**：唯一的环境变量入口。所有其他文件 `import { env } from '@/platform/env'`，禁止直接用 `process.env.*`（除 env.ts 内部）。
- **src/platform/db/client.ts**：按 `env.DB_DRIVER` 决定 import `schema.sqlite.ts` 还是 `schema.pg.ts`。Phase 0 只实现 SQLite。
- **src/backend/a2a-core/server-helpers.ts**：封装 `createA2AStreamResponse`——包装 SSE 协议细节。Phase 0 是 toy 版，Phase 2 会正规化。
- **src/app/api/agents/[agentId]/message/stream/route.ts**：spec 里写的是 `message:stream`（冒号），但**Next.js 路由里冒号有特殊语义（动态段前缀）**。Phase 0 采用 `message/stream`（斜杠），spec 里的 URL 例子需要对应修正——**这一点在本 plan 的 Task 17 会特别说明**。
...
- [x] **Step 4: 创建 src/app/globals.css**
...
```bash
git add next.config.ts next-env.d.ts tailwind.config.ts postcss.config.mjs src/app/ eslint.config.mjs
...
## Task 4: 环境变量类型化读取（src/platform/env.ts）
...
describe('lib/env', () => {
  ...
  it('loads valid env', async () => {
    ...
    const { loadEnv } = await import('@/platform/env')
    const env = loadEnv()
...
Expected: 失败，提示 `Cannot find module '@/platform/env'` 或类似。

- [x] **Step 3: 写 src/platform/env.ts 最小实现**
...
```bash
git add src/platform/env.ts tests/lib/env.test.ts
...
- Create: `src/platform/db/schema.sqlite.ts`
- Create: `src/platform/db/client.ts`
- Create: `tests/src/platform/db/client.test.ts`
...
describe('src/platform/db/client (sqlite)', () => {
  beforeAll(() => {
    process.env.NODE_ENV = 'test'
    process.env.BASE_URL = 'http://localhost:3000'
    process.env.DB_DRIVER = 'sqlite'
  ...
  it('imports sqlite schema when DB_DRIVER=sqlite', async () => {
    const { apiProfiles } = await import('@/platform/db/schema.sqlite')
...
Run:
```bash
npm test tests/src/platform/db/client.test.ts
```
...
export default {
  schema: env.DB_DRIVER === 'sqlite'
    ? './src/platform/db/schema.sqlite.ts'
    : './src/platform/db/schema.pg.ts',
...
Expected:
- `db:generate` 在 `src/platform/db/migrations/` 下生成 `0000_*.sql`
...
在 `tests/src/platform/db/client.test.ts` 末尾追加（`describe` 同一个块内）：
...
```bash
git add drizzle.config.ts src/platform/db/ tests/src/platform/db/ .gitignore
...
**Files:**
- Create: `src/platform/redis/client.ts`
- Create: `tests/src/platform/redis/client.test.ts`
...
describe('src/platform/redis/client', () => {
  let redis: RedisClient

  beforeAll(async () => {
    process.env.REDIS_URL = 'redis://localhost:6379'
    const mod = await import('@/platform/redis/client')
    redis = mod.redis
    await redis.flushdb()
  })
...
Run:
```bash
npm test tests/src/platform/redis/client.test.ts
```
...
```bash
git add src/platform/redis/ tests/src/platform/redis/
git commit -m "feat(p0): ioredis singleton client"
```

---

## Task 8: 结构化日志（src/platform/telemetry/logger.ts）

**Files:**
- Create: `src/platform/telemetry/logger.ts`
- Create: `tests/src/platform/telemetry/logger.test.ts`
...
describe('src/platform/telemetry/logger', () => {
...
Run:
```bash
npm test tests/src/platform/telemetry/logger.test.ts
```
...
```bash
git add src/platform/telemetry/ tests/src/platform/telemetry/
...
**Files:**
- Create: `src/platform/llm/catalog.ts`
- Create: `tests/src/platform/llm/catalog.test.ts`
...
```typescript
import { describe, it, expect } from 'vitest'
import { PROVIDER_CATALOG, findProvider } from '@/platform/llm/catalog'

describe('src/platform/llm/catalog', () => {
...
Run:
```bash
npm test tests/src/platform/llm/catalog.test.ts
```
...
```bash
git add src/platform/llm/catalog.ts tests/src/platform/llm/catalog.test.ts
...
**Files:**
- Create: `src/platform/llm/provider-factory.ts`
- Create: `tests/src/platform/llm/provider-factory.test.ts`
...
describe('src/platform/llm/provider-factory', () => {
  it('createModel returns a language model for openai-compatible', async () => {
    const { createModel } = await import('@/platform/llm/provider-factory')
...
Run:
```bash
npm test tests/src/platform/llm/provider-factory.test.ts
```
...
```bash
git add src/platform/llm/provider-factory.ts tests/src/platform/llm/provider-factory.test.ts
...
```bash
git add src/app/api/health/ tests/api/
...
**Files:**
- Create: `src/app/api/llm/ping/route.ts`
...
describe('POST /api/llm/ping (mocked)', () => {
  ...
  it('returns SSE stream concatenating to expected text', async () => {
    ...
    vi.doMock('@/platform/llm/provider-factory', () => ({
      createModel: () => ({
...
```bash
git add src/app/api/llm/ping/ tests/api/llm-ping.test.ts
...
- Create: `src/backend/a2a-core/server-helpers.ts`
- Create: `tests/src/backend/a2a-core/server-helpers.test.ts`
...
Run:
```bash
npm test tests/src/backend/a2a-core/server-helpers.test.ts
```
...
```bash
git add src/backend/a2a-core/types.ts src/backend/a2a-core/server-helpers.ts tests/src/backend/a2a-core/
...
- Create: `tests/src/backend/a2a-core/client.test.ts`
...
Run:
```bash
npm test tests/src/backend/a2a-core/client.test.ts
```
...
```bash
git add src/backend/a2a-core/client.ts tests/src/backend/a2a-core/client.test.ts
...
**Files:**
- Create: `src/app/api/agents/[agentId]/.well-known/agent-card.json/route.ts`
...
Run:
```bash
mkdir -p "src/app/api/agents/[agentId]/.well-known/agent-card.json"
```
...
```bash
git add src/app/api/agents/ tests/api/agent-card.test.ts
...
**Files:**
- Create: `src/app/api/agents/[agentId]/message/stream/route.ts`
...
Run:
```bash
mkdir -p "src/app/api/agents/[agentId]/message/stream"
```
...
```bash
git add src/app/api/agents/[agentId]/message/ tests/api/agent-message-stream.test.ts
...
- Modify: `src/backend/a2a-core/client.ts`（小修：支持从 agentId 推导 URL，方便测试）
...
describe('M2: A2A end-to-end (toy client → toy server)', () => {
  ...
  it('client gets fold decision from toy-poker', async () => {
    ...
    const { requestAgentDecisionToy } = await import('@/backend/a2a-core/client')
    const thoughts: string[] = []
...
### Lib 层
- ✅ `src/platform/env.ts` — zod 校验的环境变量加载
- ✅ `src/platform/db/client.ts` + `schema.sqlite.ts` — Drizzle + SQLite 驱动
- ✅ `src/platform/redis/client.ts` — ioredis 单例
- ✅ `src/platform/telemetry/logger.ts` — 结构化 JSON 日志
- ✅ `src/platform/llm/catalog.ts` — 静态 provider catalog
- ✅ `src/platform/llm/provider-factory.ts` — Vercel AI SDK 工厂（openai-compatible + anthropic）
- ✅ `src/backend/a2a-core/types.ts` — A2A SSE 事件类型
- ✅ `src/backend/a2a-core/server-helpers.ts` — `createA2AStreamResponse`（toy）
- ✅ `src/backend/a2a-core/client.ts` — `requestAgentDecisionToy`（toy）
...
2. **`@ai-sdk/openai-compatible` 调用形式**：plan 假设 `createOpenAICompatible({name, baseURL, apiKey})(model)` 返回 `LanguageModel`。若 SDK 是 `.chat(model)` / `.languageModel(model)`，调整 `src/platform/llm/provider-factory.ts`。
