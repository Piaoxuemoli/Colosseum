...
| 2 | 技术栈 | React + Next.js App Router + TypeScript + Tailwind CSS + shadcn/ui 本地 primitives + Radix UI + lucide-react icons |
...
| 14 | UI 渲染 | award-grade arena console：深色竞技场控制台、即时导航反馈、Radix 可访问 primitives、lucide 图标、适度 Framer Motion |
...
┌─────────────────────────────────────────────────────────────────┐
│ L1  UI 层                                                         │
│     src/app/ 下的 Server/Client 组件（除 src/app/api/）           │
│     Tailwind + Framer Motion                                      │
│     职责: 呈现、采集输入、订阅 SSE、无业务逻辑                    │
├─────────────────────────────────────────────────────────────────┤
│ L2  Route Handler 层（HTTP 边界）                                 │
│     src/app/api/ 下 GET/POST 函数                                 │
...
├─────────────────────────────────────────────────────────────────┤
│ L4  游戏域层（每游戏一套，游戏自治）                              │
│     src/games/poker/{engine,agent,memory,ui}                      │
│     src/games/werewolf/{engine,agent,memory,ui}                   │
├─────────────────────────────────────────────────────────────────┤
│ L5  基础设施层                                                    │
│     src/platform/db/                                              │
│     src/platform/redis/                                           │
...
Game Master self-triggered tick loop (L3)
  while !complete:
    ...
    if boundary == hand-end: updateMemory(); start next hand unless stopRequested
    if boundary == match-end: finalize()
  ↓
用户请求本手后结束或手动停止 → match status=completed → SSE terminal → RankingPanel
```
...
(B) Agent Server（每个 Agent 一套端点）
    src/app/api/agents/[agentId]/message:send/route.ts    non-streaming
    src/app/api/agents/[agentId]/message:stream/route.ts  streaming (SSE)
    src/app/api/agents/[agentId]/tasks/[taskId]/route.ts  task query
...
```typescript
// src/platform/engine/contracts.ts
...
**德扑不设 Moderator**。应用层校验：当 `gameType='poker'` 时，新建 Agent 强制 `kind='player'`；当 `gameType='werewolf'` 且 `kind='moderator'` 时，新建 match 时必须为该 match 指定 moderator agent。DB 层 enum 不强制这一约束，由 `src/backend/orchestrator/match-lifecycle.ts` 做 validation。
...
每个游戏 `src/games/<game>/agent/` 必须实现：
...
export async function tickMatch(matchId: string): Promise<TickResult> {
  ...
  try {
    ...
    if (boundary === 'hand-end') {
      await updateMemoryOnHandEnd(matchId, nextState)
      await continueAfterBoundary(matchId, nextState, 'hand-end')
    }
...
// Route Handler
// src/app/api/matches/[matchId]/tick/route.ts
...
```typescript
// src/frontend/store/profile-store.ts - localStorage persist
type ProfileStore = {
...
// src/frontend/store/match-view-store.ts - session 级
type MatchViewStore = {
  matchId: string
  events: GameEvent[]
...
按需复制（不全量安装）：Button / Input / Dialog / Select / Tabs / Card / Avatar / Badge / Progress / Tooltip。复制到 `src/frontend/components/ui/`。所有 primitives 必须服务于控制台体验：紧凑、可扫读、点击后 100ms 内有 active / pending / skeleton 反馈。

视觉参考采用 Awwwards 的优秀交互站点作为“质量标尺”，只吸收层级、微交互、导航细节和精选感；不把 WebGL 炫技、滚动叙事或营销型 hero 带入核心观战页。
...
| Engine validation | action 非法 | coerce / fallback；德扑中“可 check 时 fold”按 free fold 容错为 `check` | `layer='validate'` |
...
├── src/games/poker/engine/__tests__/
│     shuffle / 发牌 / action apply / evaluator / equity / pot-manager
├── src/games/werewolf/engine/__tests__/
│     角色分配 / 夜间交互 / 发言队列 / 胜利判定 / 边界
├── src/platform/memory/__tests__/ EMA 数学 / 200 条清理
├── src/backend/orchestrator/__tests__/ tick 推进 / 三层 fallback / 边界触发
└── src/games/**/agent/__tests__/ Response parser 模糊匹配 / 超额截断
...
**里程碑**：6 个 LLM Agent 在持续德扑桌自主打多手牌，观众看到实时思考链 + 筹码图；用户请求本手后结束时看到最终排名。
